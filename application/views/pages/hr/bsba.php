		...
		<div class="col col-3 border">
			<div class="row mt-5 p-3">
				<div id="list-example" class="list-group">
				  ...
				  <a class="list-group-item list-group-item-action bg-success text-light" href="<?= site_url('hr/bsba') ?>">BSBA</a>
		...
		<div class="col col-9 mt-5">
			<div class="card">
				...
				<div class="card-body">
					<div class="row">
						<div class="col">
							<h5 class="card-text">Male: <?= count($bsba_getmale) ?></h5>
							<h5 class="card-text">Female: <?= count($bsba_getfemale) ?></h5>
						</div>
						<div class="col">
							<canvas id="bsbagender" width="200" height="100"></canvas>	
						</div>
					</div>	
				</div>
			</div>
			<div class="card mt-2">
			    ...
			    <div class="card-body">
			    	<div class="row">
			    		<div class="col">
			    			<h5 class="card-text">Doctorate: <?= count($bsba_getdoctorate) ?></h5>
					      	<h5 class="card-text">Master's: <?= count($bsba_getmaster) ?></h5>
					      	<h5 class="card-text">Bachelor's: <?= count($bsba_getbachelor) ?></h5>
			    		</div>
			    		<div class="col">
							<canvas id="bsbaeducation" width="200" height="100"></canvas>
			    		</div>
			    	</div>
			  	</div>
			</div>
			<div class="card mt-2">
			    ...
			    <div class="card-body">
			    	<div class="row">
			    		<div class="col">
			    			<h5 class="card-text">Permanent: <?= count($bsba_getpermanent) ?></h5>
					      	<h5 class="card-text">Temporary Permanent: <?= count($bsba_gettemppermanent) ?></h5>
					      	<h5 class="card-text">Contract of Service: <?= count($bsba_getcos) ?></h5>
			    		</div>
			    		<div class="col">
							<canvas id="bsbaemploymentstat" width="200" height="100"></canvas>
			    		</div>
			    	</div>
			  	</div>
			</div>
			<div class="card mt-2">
			    ...
			    <div class="card-body">
			    	<div class="row">
			    		<div class="col">
							<h5 class="card-text">Professor: <?= count($bsba_getprofessor) ?></h5>
							<h5 class="card-text">Associate Professor: <?= count($bsba_getasscprofessor) ?></h5>
							<h5 class="card-text">Assistant Professor: <?= count($bsba_getassiprofessor) ?></h5>
							<h5 class="card-text">Instructor: <?= count($bsba_instructor) ?></h5>
			    		</div>
			    		<div class="col">
							<canvas id="bsbaacadrank" width="200" height="100"></canvas>
			    		</div>
			    	</div>
			  	</div>
			</div>
...
<script>
	const ctxbsb = document.getElementById('bsbagender').getContext('2d');
	const myChart = new Chart(ctxbsb, {
	    type: 'bar',
	    data: {
	        labels: ['Male', 'Female'],
	        datasets: [{
	            label: '',
	            data: [<?= count($bsba_getmale) ?>, <?= count($bsba_getfemale) ?>],
	            backgroundColor: [
	                '#3b76ff',
	                '#ff3b58'
	            ],
	...
	const ctxbsbaeducation = document.getElementById('bsbaeducation').getContext('2d');
	const myCharta = new Chart(ctxbsbaeducation, {
	    type: 'bar',
	    data: {
	        labels: ['Doctorate', "Master's", "Bachelor's"],
	        datasets: [{
	            label: '',
	            data: [<?= count($bsba_getdoctorate) ?>, <?= count($bsba_getmaster) ?>, <?= count($bsba_getbachelor) ?>],
	            backgroundColor: [
	                '#3b76ff',
	                '#ff3b58',
	                '#1e8a1e'
	...
	const ctxbsbaemploymentstat = document.getElementById('bsbaemploymentstat').getContext('2d');
	const myChartb = new Chart(ctxbsbaemploymentstat, {
	    type: 'bar',
	    data: {
	        labels: ['Permanent', "Temporary Permanent", "Contract of Service"],
	        datasets: [{
	            label: '',
	            data: [<?= count($bsba_getpermanent) ?>, <?= count($bsba_gettemppermanent) ?>, <?= count($bsba_getcos) ?>],
	            backgroundColor: [
	                '#3b76ff',
	                '#ff3b58',
	                '#1e8a1e'
	...
	const ctxbsbaacadrank = document.getElementById('bsbaacadrank').getContext('2d');
	const myChartc = new Chart(ctxbsbaacadrank, {
	    type: 'bar',
	    data: {
	        labels: ['Professor', "Associate Professor", "Assistant Professor", "Instructor"],
	        datasets: [{
	            label: '',
	            data: [<?= count($bsba_getprofessor) ?>, <?= count($bsba_getasscprofessor) ?>, <?= count($bsba_getassiprofessor) ?>, <?= count($bsba_instructor) ?>],
	            backgroundColor: [
	                '#3b76ff',
	                '#ff3b58',
	                '#1e8a1e',
